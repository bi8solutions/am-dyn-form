import {ConnectableObservable, defer, Observable, pipe, ReplaySubject, Subscribable, Subscription, SubscriptionLike, Unsubscribable} from "rxjs";
...
export namespace Channels {
  ...
  export abstract class BaseChannel<A = {}, T = {}, D = {}> implements IChannel<A, T, D> {
    ...
    disable(): Channels.IChannel<A, T, D> {
      this.enabled = false;
      return this;
    }

    enable(): Channels.IChannel<A, T, D> {
      this.enabled = true;
      return this;
    }
    ...
    abstract subscribe(observerOrNext?: PartialObserver<T> |
  ...
  export interface ChannelOptions<A = {}, T = {}, D = {}> {
    key?: any,
    deferOptions?: CreateOptions<A, T, D>,
    input?: Subject<A>,
    output?: Subject<T>,
    debug?: DebugOptions;
    eventFn?: EventFn,
  ...
  export class Channel<A = {}, T = {}, D = {}> extends BaseChannel<A, T, D> {
    ...
    constructor(options?: ChannelOptions<A, T, D>){
      if (options){
        ...
        this.createOptions = options.deferOptions;
    ...
    protected setupSubjects() {
      ...
      this.input$.pipe(
        ...
        tap(()=>this.emitEvent({observeType: EventType.on_next})),
        ...
        next: (value: any)=>{
          try {
            ...
            let observable = this.createOptions
              ? actionObservable(this.createOptions) : new Observable((sub)=>{ sub.next(value); sub.complete(); });
            ...
            if (this.cold) {
              ...
              observable.pipe(shieldsUp, takeMeOn, eventTappet).subscribe({ next: () => this.output$.next(value), error: (err) => {}, complete: () => {} });
            }
          } catch(err){
    ...
    subscribe(observerOrNext?: PartialObserver<T> | ((value: T) => void), error?: (error: any) => void, complete?: () => void, ...ops: OperatorFunction<any, any>[]): Subscription {
  ...
  export class ChannelSwitch {
    ...
    add(...channels: Channel[]){
      channels.forEach((channel)=>{ this.channelMap.set(channel.key, channel); })
    }

    remove(...keys: any[]){
      keys.forEach((key)=>{ this.channelMap.delete(key); })
    }

    complete(...keys: any[]){
      keys.forEach((key)=>{ let channel = this.channelMap.get(key); if (channel){ channel.complete()}});
    }

    enable(...keys: any[]){
      keys.forEach((key)=>{ let channel = this.channelMap.get(key); if (channel){ channel.enable()}});
    }

    disable(...keys: any[]){
      keys.forEach((key)=>{ let channel = this.channelMap.get(key); if (channel){ channel.disable()}});
    }

    close(){
      Array.from(this.channelMap.entries()).forEach((entry: [any, Channel])=>{ entry[1].close()});
    }
  }
   ...
   * @param arg
   */
  export function actionObservable<A = {}, T = {}, D = {}>(arg: CreateOptions<A, T, D> | ICreateFn<A, T>) : Observable<T> {
    if (arg instanceof CreateOptions){
      ...
      return options.defer ? defer(()=> options.map ? options.new(options.map()) : options.new())
                           : options.map ? options.new(options.map()) : options.new();
    } else {
